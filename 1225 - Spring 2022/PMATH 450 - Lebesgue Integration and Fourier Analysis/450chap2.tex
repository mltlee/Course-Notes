...
\subsection{Lebesgue Measurable Functions}\label{subsec:2.4}
...
\begin{remark}{remark:2.24}
    ...
    functions via open sets. Suppose that $f : D \to \R$ is continuous 
    where $D \in {\cal L}$. Then for every open subset $U \subseteq \R$, 
    we know that $V = f^{-1}(U)$ is open in $D$. Then $V \cap D$ is open in 
    $\R$ and $V \in {\cal L}$ since it is a Borel set, so $f$ is Lebesgue 
    measurable. 
\end{remark}
