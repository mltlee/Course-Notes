...
\begin{prop}{prop:2.4}
    ...
    \{I_n\}_{n=1}^\infty \text{ a cover of open intervals such that }
    \ell(I_n) < \delta \right\}. \] 
\end{prop}
\begin{pf}
...
\begin{pf}
    ...
    Let $\eps > 0$. By taking $I_1 = (a - \eps/2, b + \eps/2)$
    and $I_2 = \varnothing$ for $n \geq 2$, the collection $\{I_n\}_{n=1}^\infty$ 
    is a cover of $[a, b]$ by intervals, which implies that 
    \[ m^*(I) \leq \sum_{n=1}^\infty \ell(I_n) = \ell(I_1) = (b-a) + \eps. \] 
    Since $\eps > 0$ is arbitrary, we have $m^*(I) \leq b-a$. 

    We now turn to showing that $m^*(I) \geq b-a$. Let $\eps > 0$. By 
    Proposition~\ref{prop:2.4}, we can find an open cover $\bigcup_{n=1}^\infty 
    (a_n, b_n)$ of $I$ such that 
    ...
    $0 < \eps < b-a$, we have that $[a + \eps, b] \subseteq (a, b] \subseteq 
